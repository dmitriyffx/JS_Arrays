...
// Добавьте к задаче возможность отправлять размер скидки в процентах.


function task_closure_3() {
    ...
    function showOrder(action) {
        let str = `Заказ покупателя ${this.fio} составил ${this.price * this.quantity*(100-action)/100}, скидка сегодня составила ${action}%`;
        console.log(str);
    }

    orders.forEach(elem => showOrder.call(elem, 0));
    orders.forEach(elem => showOrder.call(elem, 10));
    orders.forEach(elem => showOrder.call(elem, 20));
